# -*- coding: utf-8 -*-
"""
Created on Fri Apr 11 16:03:14 2025
...
# Calculate number of complete days and trim
nt = len(df_in) // 24
df_in = df_in.iloc[:nt * 24]
df_out = df_out.iloc[:nt]

# Add VPD and select input features
...
df_in_features = df_in.drop(columns=['timestamp']).astype(np.float32)

df_out = df_out.mean(axis=1)
# Keep df_out as is (no averaging, no padding)
#df_out = df_out.reset_index(drop=True).astype(np.float32)

# Repeat each daily row 24 times to match hourly steps
df_out_expanded = df_out.loc[df_out.index.repeat(24)].reset_index(drop=True)

# Trim to match df_in_features length
df_out = df_out_expanded.iloc[:len(df_in_features)]

# Process and align soil moisture
df_sm_T4N = pd.read_csv('C:/Users/Collin/SUMMA/SUMMA_shared/sapflux/T4n_combine1.csv')
df_sm_T4N = df_sm_T4N[['TIMESTAMP', 'VWC_1_Avg', 'VWC_2_Avg', 'VWC_3_Avg']]
...
# Slice and filter soil moisture
...
df_in = pd.concat([df_in_features, df_sm[['VWC_1_Avg', 'VWC_2_Avg', 'VWC_3_Avg']]], axis=1)

# Output:
# df_combined → full hourly input features
# df_out_expanded → matching hourly target per sensor (with multiple columns still present)
