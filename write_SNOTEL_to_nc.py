...
def calculate_specific_humidity(relative_humidity, air_temperature, pressure):
    ...
    q = (0.622 * e) / (pressure - 0.378 * e)  # Specific humidity in kg/kg
    q = q*1000
    return q.fillna(0)
        ...
        with nc.Dataset(new_file, 'w', format='NETCDF4') as dst:
            ...
            for name, var in src.variables.items():
                ...
                if name in var_names:
                    new_var[:] = new_data_list[var_names.index(name)]
                else:
                    new_var[:] = var[:]
...
f_sagehen = ('C:/Users/Collin/Documents/SUMMA/SUMMA_shared/'
             'Lisa_s_SNOWPACK_Historical_analysis/'+str(elev)+'/input/SNOTEL_'+str(SNOTEL_station)+'.SMET')
...
SpecHum = calculate_specific_humidity(df_sagehen['RH'], df_sagehen['TA'], 1050)
...
copy_and_modify_netcdf(file_path, ddir+'sagehen/forcing_sagehen_'+str(elev)+'.nc', ['latitude','longitude','time','LWRadAtm','SWRadAtm','airtemp','pptrate','spechum'], [lat,lon,df_sagehen['timestamp'],df_sagehen['ILWR'],df_sagehen['ISWR'],df_sagehen['TA'],df_sagehen['Prate'],df_sagehen['SpecHum']])
